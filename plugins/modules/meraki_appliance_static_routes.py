...
# See plugins/action/meraki_appliance_static_routes.py for the implementation.

DOCUMENTATION = r'''
module: meraki_appliance_static_routes

short_description: Manage Meraki appliance static routes

description:
  - Manage Meraki appliance static routes for a network.
  - Supports merged, replaced, deleted, and gathered states.

notes:
  - "Canonical key: C(name) — identifies the resource in playbooks."
  - "System key: C(static_route_id) — server-assigned, resolved automatically from gathered state."
  - "Users do not need to provide C(static_route_id) unless disambiguating duplicate names."
...
options:
  ...
  config:
    ...
    suboptions:
      ...
      subnet:
        description: Subnet of the route (e.g., 192.168.1.0/24).
        type: str
...
EXAMPLES = r'''
---
# Manage Meraki appliance static routes — create or update

- name: Define expected configuration
  ansible.builtin.set_fact:
    expected_config:
      static_route_id: example
      name: Test-Config
...
- name: Create appliance_static_routes with merged state
...
# Manage Meraki appliance static routes — full resource replacement
...
- name: Replace appliance_static_routes configuration
  cisco.meraki_rm.meraki_appliance_static_routes:
    network_id: "N_123456789012345678"
    state: replaced
    config:
...
# Manage Meraki appliance static routes — override all instances
...
- name: Override all appliance_static_routes — desired state only
  cisco.meraki_rm.meraki_appliance_static_routes:
    network_id: "N_123456789012345678"
    state: overridden
    config:
...
# Manage Meraki appliance static routes — gather current configuration

- name: Gather current appliance_static_routes configuration
  cisco.meraki_rm.meraki_appliance_static_routes:
    network_id: "N_123456789012345678"
    state: gathered
  register: gathered
...
# Manage Meraki appliance static routes — remove configuration
...
- name: Delete appliance_static_routes configuration
  cisco.meraki_rm.meraki_appliance_static_routes:
    network_id: "N_123456789012345678"
    state: deleted
    config:
