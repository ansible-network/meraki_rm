...
# See plugins/action/meraki_appliance_prefixes.py for the implementation.

DOCUMENTATION = r'''
module: meraki_appliance_prefixes

short_description: Manage Meraki appliance delegated IPv6 prefixes
...
options:
  ...
  config:
    description: List of prefix configurations.
    type: list
    elements: dict
    suboptions:
      ...
      origin:
        description: WAN1/WAN2/Independent prefix configuration.
        type: dict
'''

EXAMPLES = r'''
---
# Manage Meraki appliance delegated IPv6 prefixes — create or update

- name: Define expected configuration
  ansible.builtin.set_fact:
    expected_config:
      static_delegated_prefix_id: example
      prefix: example
      description: Managed by Ansible

- name: Create appliance_prefixes with merged state
  cisco.meraki_rm.meraki_appliance_prefixes:
...
# Manage Meraki appliance delegated IPv6 prefixes — full resource replacement
...
- name: Replace appliance_prefixes configuration
  cisco.meraki_rm.meraki_appliance_prefixes:
    network_id: "N_123456789012345678"
    state: replaced
    config:
...
# Manage Meraki appliance delegated IPv6 prefixes — override all instances
...
- name: Override all appliance_prefixes — desired state only
  cisco.meraki_rm.meraki_appliance_prefixes:
    network_id: "N_123456789012345678"
    state: overridden
    config:
...
# Manage Meraki appliance delegated IPv6 prefixes — gather current configuration

- name: Gather current appliance_prefixes configuration
  cisco.meraki_rm.meraki_appliance_prefixes:
    network_id: "N_123456789012345678"
    state: gathered
  register: gathered
...
# Manage Meraki appliance delegated IPv6 prefixes — remove configuration
...
- name: Delete appliance_prefixes configuration
  cisco.meraki_rm.meraki_appliance_prefixes:
    network_id: "N_123456789012345678"
    state: deleted
    config:
