...
# See plugins/action/meraki_vlan_profiles.py for the implementation.

DOCUMENTATION = r'''
module: meraki_vlan_profiles

short_description: Manage Meraki VLAN profiles
...
options:
  ...
  config:
    description: List of VLAN profile configurations.
    type: list
    elements: dict
    suboptions:
      iname:
        description: VLAN profile iname (primary key). Required for merged, replaced, deleted.
        type: str

      name:
        description: Name of the profile (1-255 chars).
        type: str

      is_default:
        description: Whether this is the default VLAN profile.
        type: bool

      vlan_names:
        description: Array of named VLANs.
        type: list
        elements: dict

      vlan_groups:
        description: Array of named VLAN groups.
        type: list
        elements: dict

      vlan_profile:
        description: VLAN profile configuration.
        type: dict
'''

EXAMPLES = r'''
---
# Manage Meraki VLAN profiles — create or update
...
- name: Create vlan_profiles with merged state
  cisco.meraki_rm.meraki_vlan_profiles:
...
# Manage Meraki VLAN profiles — full resource replacement
...
- name: Replace vlan_profiles configuration
  cisco.meraki_rm.meraki_vlan_profiles:
    network_id: "N_123456789012345678"
    state: replaced
    config:
...
# Manage Meraki VLAN profiles — override all instances
...
- name: Override all vlan_profiles — desired state only
  cisco.meraki_rm.meraki_vlan_profiles:
    network_id: "N_123456789012345678"
    state: overridden
    config:
...
# Manage Meraki VLAN profiles — gather current configuration

- name: Gather current vlan_profiles configuration
  cisco.meraki_rm.meraki_vlan_profiles:
    network_id: "N_123456789012345678"
    state: gathered
  register: gathered
...
# Manage Meraki VLAN profiles — remove configuration
...
- name: Delete vlan_profiles configuration
  cisco.meraki_rm.meraki_vlan_profiles:
    network_id: "N_123456789012345678"
    state: deleted
    config:
