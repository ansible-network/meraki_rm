"""Drift-detection tests for generated VlanProfile dataclass."""
...
class TestSchema:
    """VlanProfile field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(vlan_profile.VlanProfile)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(vlan_profile.VlanProfile())}
        expected = {'vlanGroups', 'productType', 'vlanProfile', 'mac', 'stackIds', 'isDefault', 'serial', 'iname', 'name', 'vlanNames', 'serials', 'stack'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = vlan_profile.VlanProfile()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(vlan_profile.VlanProfile())) == 12


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['iname', 'isDefault', 'mac', 'name', 'productType', 'serial', 'serials', 'stack', 'stackIds', 'vlanGroups', 'vlanNames', 'vlanProfile']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(vlan_profile.VlanProfile())}
