"""Drift-detection tests for generated WarmSpare dataclass."""
...
class TestSchema:
    """WarmSpare field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(warm_spare.WarmSpare)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(warm_spare.WarmSpare())}
        expected = {'virtualIp1', 'enabled', 'wan2', 'spareSerial', 'primarySerial', 'uplinkMode', 'virtualIp2', 'wan1'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = warm_spare.WarmSpare()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(warm_spare.WarmSpare())) == 8


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['enabled', 'primarySerial', 'spareSerial', 'uplinkMode', 'virtualIp1', 'virtualIp2', 'wan1', 'wan2']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(warm_spare.WarmSpare())}
