"""Drift-detection tests for generated Firewall dataclass."""
...
class TestSchema:
    """Firewall field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(firewall.Firewall)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(firewall.Firewall())}
        expected = {'service', 'allowedIps', 'network', 'spoofingProtection', 'applicationCategories', 'rules', 'access', 'syslogDefaultRule'}
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(firewall.Firewall())) == 8


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['access', 'allowedIps', 'applicationCategories', 'network', 'rules', 'service', 'spoofingProtection', 'syslogDefaultRule']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(firewall.Firewall())}
