"""Drift-detection tests for generated SwitchAcl dataclass."""
...
class TestSchema:
    """SwitchAcl field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(switch_acl.SwitchAcl)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(switch_acl.SwitchAcl())}
        expected = {'rules'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = switch_acl.SwitchAcl()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(switch_acl.SwitchAcl())) == 1
...
class TestSpecDrift:
    ...
    def test_dataclass_covers_spec(self):
        ...
        dc_names = {f.name for f in dc_fields(switch_acl.SwitchAcl())}
