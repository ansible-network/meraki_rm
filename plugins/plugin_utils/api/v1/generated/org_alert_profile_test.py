"""Drift-detection tests for generated OrgAlertProfile dataclass."""
...
class TestSchema:
    """OrgAlertProfile field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(org_alert_profile.OrgAlertProfile)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(org_alert_profile.OrgAlertProfile())}
        expected = {'enabled', 'networkTags', 'id', 'type', 'recipients', 'alertCondition', 'description'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = org_alert_profile.OrgAlertProfile()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(org_alert_profile.OrgAlertProfile())) == 7


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['alertCondition', 'description', 'enabled', 'id', 'networkTags', 'recipients', 'type']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(org_alert_profile.OrgAlertProfile())}
