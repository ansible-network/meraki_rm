"""Drift-detection tests for generated PolicyObject dataclass."""
...
class TestSchema:
    """PolicyObject field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(policy_object.PolicyObject)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(policy_object.PolicyObject())}
        expected = {'id', 'cidr', 'groupIds', 'objectIds', 'type', 'name', 'category', 'ip', 'createdAt', 'updatedAt', 'mask', 'fqdn', 'networkIds'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = policy_object.PolicyObject()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(policy_object.PolicyObject())) == 13


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['category', 'cidr', 'createdAt', 'fqdn', 'groupIds', 'id', 'ip', 'mask', 'name', 'networkIds', 'objectIds', 'type', 'updatedAt']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(policy_object.PolicyObject())}
