"""Drift-detection tests for generated SwitchQosRule dataclass."""
...
class TestSchema:
    """SwitchQosRule field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(switch_qos_rule.SwitchQosRule)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(switch_qos_rule.SwitchQosRule())}
        expected = {'srcPortRange', 'id', 'srcPort', 'protocol', 'ruleIds', 'dstPort', 'vlan', 'dscp', 'dstPortRange'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = switch_qos_rule.SwitchQosRule()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(switch_qos_rule.SwitchQosRule())) == 9


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['dscp', 'dstPort', 'dstPortRange', 'id', 'protocol', 'ruleIds', 'srcPort', 'srcPortRange', 'vlan']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(switch_qos_rule.SwitchQosRule())}
