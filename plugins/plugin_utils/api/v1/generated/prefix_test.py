"""Drift-detection tests for generated Prefix dataclass."""
...
class TestSchema:
    """Prefix field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(prefix.Prefix)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(prefix.Prefix())}
        expected = {'origin', 'updatedAt', 'description', 'createdAt', 'staticDelegatedPrefixId', 'prefix'}
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(prefix.Prefix())) == 6


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['createdAt', 'description', 'origin', 'prefix', 'staticDelegatedPrefixId', 'updatedAt']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(prefix.Prefix())}
