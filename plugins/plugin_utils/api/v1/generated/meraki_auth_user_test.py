"""Drift-detection tests for generated MerakiAuthUser dataclass."""
...
class TestSchema:
    """MerakiAuthUser field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(meraki_auth_user.MerakiAuthUser)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(meraki_auth_user.MerakiAuthUser())}
        expected = {'isAdmin', 'id', 'accountType', 'name', 'emailPasswordToUser', 'authorizations', 'createdAt', 'email', 'password'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = meraki_auth_user.MerakiAuthUser()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(meraki_auth_user.MerakiAuthUser())) == 9


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['accountType', 'authorizations', 'createdAt', 'email', 'emailPasswordToUser', 'id', 'isAdmin', 'name', 'password']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(meraki_auth_user.MerakiAuthUser())}
