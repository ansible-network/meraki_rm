"""Drift-detection tests for generated SensorAlertProfile dataclass."""
...
class TestSchema:
    """SensorAlertProfile field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(sensor_alert_profile.SensorAlertProfile)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(sensor_alert_profile.SensorAlertProfile())}
        expected = {'schedule', 'conditions', 'includeSensorUrl', 'message', 'name', 'recipients', 'serials', 'profileId'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = sensor_alert_profile.SensorAlertProfile()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(sensor_alert_profile.SensorAlertProfile())) == 8


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['conditions', 'includeSensorUrl', 'message', 'name', 'profileId', 'recipients', 'schedule', 'serials']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(sensor_alert_profile.SensorAlertProfile())}
