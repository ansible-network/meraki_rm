"""Drift-detection tests for generated CameraWirelessProfile dataclass."""
...
class TestSchema:
    """CameraWirelessProfile field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(camera_wireless_profile.CameraWirelessProfile)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(camera_wireless_profile.CameraWirelessProfile())}
        expected = {'ssid', 'id', 'identity', 'appliedDeviceCount', 'name'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = camera_wireless_profile.CameraWirelessProfile()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(camera_wireless_profile.CameraWirelessProfile())) == 5


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['appliedDeviceCount', 'id', 'identity', 'name', 'ssid']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(camera_wireless_profile.CameraWirelessProfile())}
