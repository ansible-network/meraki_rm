"""Drift-detection tests for generated EthernetPortProfile dataclass."""
...
class TestSchema:
    """EthernetPortProfile field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(ethernet_port_profile.EthernetPortProfile)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(ethernet_port_profile.EthernetPortProfile())}
        expected = {'usbPorts', 'ports', 'isDefault', 'name', 'serials', 'profileId'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = ethernet_port_profile.EthernetPortProfile()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(ethernet_port_profile.EthernetPortProfile())) == 6


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['isDefault', 'name', 'ports', 'profileId', 'serials', 'usbPorts']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(ethernet_port_profile.EthernetPortProfile())}
