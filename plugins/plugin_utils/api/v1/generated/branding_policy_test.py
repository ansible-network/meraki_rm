"""Drift-detection tests for generated BrandingPolicy dataclass."""
...
class TestSchema:
    """BrandingPolicy field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(branding_policy.BrandingPolicy)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(branding_policy.BrandingPolicy())}
        expected = {'enabled', 'adminSettings', 'customLogo', 'name', 'helpSettings', 'brandingPolicyIds'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = branding_policy.BrandingPolicy()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(branding_policy.BrandingPolicy())) == 6


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['adminSettings', 'brandingPolicyIds', 'customLogo', 'enabled', 'helpSettings', 'name']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(branding_policy.BrandingPolicy())}
