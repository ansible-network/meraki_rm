"""Drift-detection tests for generated DeviceManagementInterface dataclass."""
...
class TestSchema:
    """DeviceManagementInterface field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(device_management_interface.DeviceManagementInterface)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(device_management_interface.DeviceManagementInterface())}
        expected = {'wan1', 'ddnsHostnames', 'wan2'}
        assert expected.issubset(field_names), f'Missing fields: {expected - field_names}'

    def test_all_fields_optional(self):
        """Every generated field should default to None (all Optional)."""
        obj = device_management_interface.DeviceManagementInterface()
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(device_management_interface.DeviceManagementInterface())) == 3


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['ddnsHostnames', 'wan1', 'wan2']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(device_management_interface.DeviceManagementInterface())}
