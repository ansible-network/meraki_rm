"""Drift-detection tests for generated Admin dataclass."""
...
class TestSchema:
    """Admin field inventory — catches regeneration drift."""

    def test_is_dataclass(self):
        assert is_dataclass(admin.Admin)

    def test_expected_fields_exist(self):
        field_names = {f.name for f in dc_fields(admin.Admin())}
        expected = {'orgAccess', 'id', 'lastActive', 'tags', 'hasApiKey', 'twoFactorAuthEnabled', 'name', 'networks', 'email', 'accountStatus', 'authenticationMethod'}
    ...
    def test_field_count(self):
        ...
        assert len(dc_fields(admin.Admin())) == 11


class TestSpecDrift:
    """Cross-reference dataclass fields against spec3.json response schemas."""

    SPEC_FIELDS = ['accountStatus', 'authenticationMethod', 'email', 'hasApiKey', 'id', 'lastActive', 'name', 'networks', 'orgAccess', 'tags', 'twoFactorAuthEnabled']

    def test_dataclass_covers_spec(self):
        """Every spec response property should have a dataclass field."""
        dc_names = {f.name for f in dc_fields(admin.Admin())}
